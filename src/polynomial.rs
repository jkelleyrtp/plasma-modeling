...
pub struct Polynomial<T = f64> {
    data: smallvec::SmallVec<[T; 11]>,
}
...
impl<T: Zero> Polynomial<T> {
    ...
    /// let poly = Polynomial::new(smallvec![1, 2, 3]);
    ...
    T: One + Zero + Clone + Neg<Output = T> + Div<Output = T> + Mul<Output = T> + Sub<Output = T>,
{
    ...
    pub fn lagrange(xs: &[T], ys: &[T]) -> Option<Self> {
        let mut res = Polynomial::new_vec(smallvec![Zero::zero()]);
        for ((i, x), y) in (0..).zip(xs.iter()).zip(ys.iter()) {
            let mut p: Polynomial<T> = Polynomial::new_vec(smallvec![T::one()]);
            let mut denom = T::one();
            for (j, x2) in (0..).zip(xs.iter()) {
                if i != j {
                    p = p * &Polynomial::new_vec(smallvec![-x2.clone(), T::one()]);
            ...
            res = res + p * &Polynomial::<T>::new_vec(smallvec![scalar]);
        }
        Some(res)
    }
}
    ...
    <T as Neg>::Output: Zero,
{
    ...
    #[inline]
    fn neg(self) -> Polynomial<<T as Neg>::Output> {
        Polynomial::new_vec(self.data.iter().map(|x| -x.clone()).collect())
    }
}
    ...
    <Lhs as Add<Rhs>>::Output: Zero,
{
    ...
    fn add(self, other: &Polynomial<Rhs>) -> Polynomial<<Lhs as Add<Rhs>>::Output> {
        ...
        let mut sum = SmallVec::with_capacity(max_len);
        for i in 0..min_len {
    ...
    <Lhs as Sub<Rhs>>::Output: Zero,
{
    ...
    fn sub(self, other: &Polynomial<Rhs>) -> Polynomial<<Lhs as Sub<Rhs>>::Output> {
        ...
        let mut sub = SmallVec::with_capacity(max_len);
        for i in 0..min_len {
    ...
    <Lhs as Mul<Rhs>>::Output: Zero,
{
    ...
    fn mul(self, other: &Polynomial<Rhs>) -> Polynomial<<Lhs as Mul<Rhs>>::Output> {
        if self.is_zero() || other.is_zero() {
            return Polynomial::new_vec(smallvec![]);
        }
...
mod tests {
    ...
    //         assert_eq!(dst, Polynomial::new_vec(src).data);
    //     }
    //     check(smallvec![1, 2, 3], smallvec![1, 2, 3]);
    //     check(smallvec![1, 2, 3], smallvec![1, 2, 3, 0, 0]);
    //     check(smallvec![], smallvec![0, 0, 0]);
    // }
    ...
    //         let a = &Polynomial::new_vec(a.to_vec());
    //         let b = &Polynomial::new_vec(b.to_vec());
    //         let c = &Polynomial::new_vec(c.to_vec());
    //         check_add(c, a, b);
    //         check_add(&(-c), &(-a), &(-b));
    //         check_sub(c, a, b);
    //         check_sub(&(-c), &(-a), &(-b));
    ...
    //         for n in -10..10 {
    //             assert_eq!(f(n), Polynomial::new_vec(pol.to_vec()).eval(n));
    //         }
    //     }
    //     check(&[], |_x| 0);
    //     check(&[1], |_x| 1);
    ...
    //         assert_eq!(
    //             s.to_string(),
    //             Polynomial::new_vec(slice.to_vec()).pretty("x")
    //         );
    //     }
    //     check(&[0], "0");
    //     check(&[1], "1");
